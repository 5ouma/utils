...
jobs:
  job:
    ...
    steps:
      - uses: 5ouma/utils/pre-commit@v0.4.11
        with:
...
jobs:
  job:
    runs-on: Ubuntu-Latest
    steps:
      - uses: 5ouma/utils/setup-bun-with-cache@v0.4.11
```
...
jobs:
  job:
    runs-on: Ubuntu-Latest
    steps:
      - uses: 5ouma/utils/update-deno-lock-file@v0.4.11
```
...
```json
{
  "extends": ["github>5ouma/utils#v0.4.11", "github>5ouma/utils:labels#v0.4.11"]
}
```
