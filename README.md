...
jobs:
  job:
    ...
    runs-on: Ubuntu-Slim
    steps:
      - uses: 5ouma/utils/label-past-pr@v0.11.0
        with:
...
jobs:
  job:
    runs-on: Ubuntu-Slim
    steps:
      - uses: 5ouma/utils/pre-commit@v0.11.0
        with:
...
jobs:
  job:
    ...
    steps:
      - uses: 5ouma/utils/setup-bun-with-cache@v0.11.0
```
...
```json
{
  "extends": ["github>5ouma/utils#v0.11.0", "github>5ouma/utils:labels#v0.11.0"]
}
```
