...
jobs:
  job:
    ...
    runs-on: Ubuntu-Slim
    steps:
      - uses: 5ouma/utils/label-past-pr@v0.11.2
        with:
...
jobs:
  job:
    runs-on: Ubuntu-Slim
    steps:
      - uses: 5ouma/utils/pre-commit@v0.11.2
        with:
...
jobs:
  job:
    ...
    steps:
      - uses: 5ouma/utils/setup-bun-with-cache@v0.11.2
```
...
```json
{
  "extends": ["github>5ouma/utils#v0.11.2", "github>5ouma/utils:labels#v0.11.2"]
}
```
